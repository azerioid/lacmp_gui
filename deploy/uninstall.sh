...
Usage: uninstall.sh [--drop-db] [--php=8.4]
  ...
  --drop-db   Drop lacmp_panel schema/users and /etc/lacmp-panel (secrets)
...
rm -f /etc/lacmp-panel/access.env
...
for _ap in /etc/apache2/sites-available/lacmp-panel.conf /etc/httpd/conf.d/vhost/lacmp-panel.conf; do
    if [[ -f "${_ap}" ]]; then
        command -v a2dissite >/dev/null 2>&1 && a2dissite lacmp-panel >/dev/null 2>&1 || true
        rm -f "${_ap}" /etc/apache2/sites-enabled/lacmp-panel.conf
        rm -f /etc/apache2/conf-available/lacmp-panel-listen.conf /etc/apache2/conf-enabled/lacmp-panel-listen.conf
        rm -f /etc/httpd/conf.d/lacmp-panel-listen.conf
...
rm -f /etc/systemd/system/caddy.service.d/lacmp-panel-reload.conf
...
if [[ -f /etc/lacmp-panel/caddy-admin-managed ]] && [[ -f /etc/caddy/Caddyfile ]]; then
...
new = re.sub(r"\n\s*# lacmp-panel: IPv4 admin[^\n]*", "", new, count=1)
if n:
    path.write_text(new)
    print("Restored Caddy admin off (panel-managed)")
PY
    rm -f /etc/lacmp-panel/caddy-admin-managed
...
if [[ -n "${PHP_VER}" ]]; then
    ...
    if [[ -n "${POOL_DIR}" ]]; then
        rm -f "${POOL_DIR}/lacmp-panel.conf"
        python3 - "${POOL_DIR}" <<'PY'
import pathlib, re, sys
...
PY
    ...
    if [[ -n "${FPM_INI:-}" && -f "${FPM_INI}.lacmp-panel.bak" && -f "${FPM_INI}" ]]; then
...
PY
    ...
    rm -f "/etc/php/${PHP_VER}/fpm/pool.d/lacmp-panel.conf" 2>/dev/null || true
    rm -f /etc/php-fpm.d/lacmp-panel.conf 2>/dev/null || true
    rm -f "/etc/systemd/system/php${PHP_VER}-fpm.service.d/lacmp-panel.conf"
    rm -f /etc/systemd/system/php-fpm.service.d/lacmp-panel.conf
...
if [[ "${DROP_DB}" -eq 0 && -f "${PREFIX}/web/.env" ]]; then
    ...
    install -m 0600 -o root -g root "${PREFIX}/web/.env" /etc/lacmp-panel/web.env
fi

rm -rf "${PREFIX}"
rm -f /var/log/caddy/access_lacmp-panel.log /var/log/caddy/lacmp-panel.log
rm -f /var/log/apache2/lacmp-panel-error.log /var/log/apache2/lacmp-panel-access.log
rm -f /var/log/httpd/lacmp-panel-error.log /var/log/httpd/lacmp-panel-access.log
rm -rf /var/lib/lacmp-panel
rm -f /var/log/lacmp-panel/auth-fail.log /var/log/lacmp-panel/php-fpm.log

# Legacy location from the first (incorrect) deploy. Only remove if it looks
# like our panel, never a random site under /data/www.
if [[ -f /data/www/lacmp-panel/web/artisan && -f /data/www/lacmp-panel/broker/broker ]]; then
    rm -rf /data/www/lacmp-panel
    echo "Removed leftover /data/www/lacmp-panel (legacy control-plane path)."
fi

if [[ "${DROP_DB}" -eq 1 ]]; then
    mariadb_admin -e "DROP DATABASE IF EXISTS lacmp_panel; DROP USER IF EXISTS 'lacmp_panel'@'localhost'; DROP USER IF EXISTS 'lacmp_panel'@'127.0.0.1'; DROP USER IF EXISTS 'lacmp_panel_admin'@'localhost'; DROP USER IF EXISTS 'lacmp_panel_admin'@'127.0.0.1'; FLUSH PRIVILEGES;"
    rm -rf /etc/lacmp-panel
    echo "Dropped panel DB users and /etc/lacmp-panel."
else
    echo "Kept /etc/lacmp-panel and the lacmp_panel database (pass --drop-db to remove)."
fi

echo "LACMP Panel removed. Existing sites were not touched."
